...
public interface ActivityMapper {
	 ...
	 */
	List<Map<String,Object>> getActTypeByOrganizationId(@Param("organizationId") int organizationId);	
	/**
	 * 通过组织ID获取组织活动日期
	 * Param organizationId	组织ID
	 * @return List<Map<String,Object>> 组织活动日期
	 */
	List<Map<String,Object>> getActDateByOrganizationId(@Param("organizationId") int organizationId);
	/**
	 * 通过组织ID获取组织活动详情
	 * Param organizationId	组织ID
	 ...
	 */
	List<Map<String,Object>> getFortsByActivityId(@Param("activityId") int activityId);
	/**
	 * 通过活动ID获取活动详情
	 * Param activityId	活动ID
	 ...
	 * @return int 操作标示 0失败其余成功
	 */
	int saveActFortScore(@Param("activityId") int activityId,@Param("memberId") int memberId);
	 ...
	 * @return int 操作标示 0失败其余成功
	 */
	int saveActSpanFortScore(@Param("activityId") int activityId,@Param("memberId") int memberId);
	/**
	 * 通过活动ID获取跨服要塞数据
	 * Param activityId	活动ID
	 * @return List<Map<String,Object>> 跨服要塞数据
	 */
	List<Map<String,Object>> getSpanFortsByActivityId(@Param("activityId") int activityId);
	 ...
	 */
	List<Map<String,Object>> getFightByActivityId(@Param("activityId") int activityId);
	/**
	 * 通过日期差值获取战力增长数据
	 * Param memberId	成员ID endDate 上次的截止日期
	 * @return MemberFight 战力增长数据
	 */
	MemberFight getMemberFightBy7Day(@Param("memberId") int memberId,@Param("endDate") Date endDate);
	/**
	 * 新增战力涨幅活动记录
	 * Param int activityId 活动ID,int memberId	成员ID
	 * @return int 操作标示 0失败其余成功
	 */
	int saveFightScore(@Param("activityId") int activityId,@Param("memberId") int memberId,@Param("beginDate") Date beginDate,
			@Param("endDate") Date endDate,@Param("memberBeginFight") Double memberBeginFight);
	 ...
	 */
	List<Map<String,Object>> getConquestByActivityId(@Param("activityId") int activityId);
	/**
	 * 更新争霸活动记录
	 * Map<String, Object> params 活动信息
	 * @return int 操作标示 0失败其余成功
	 ...
	 */
	List<Map<String,Object>> getSkyLandByActivityId(@Param("activityId") int activityId);
	/**
	 * 新增天地战场明细记录
	 * Param int activityId 活动ID,int memberId	成员ID
	 * @return int 操作标示 0失败其余成功
	 */
	int saveSkyLand(@Param("activityId") int activityId,@Param("memberId") int memberId);
	/**
	 * 更新天地活动记录
	 * Map<String, Object> params 活动信息
	 * @return int 操作标示 0失败其余成功
